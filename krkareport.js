...
async function generateMonthlyReport() {
  ...
  try {
    ...
    const textAreaLeft = hasImage ? margin + coverConfig.w + 5 : margin;
    ...
    doc.setFontSize(16);
    doc.text('Digital oglasavanje', textCenterX, ty + 3, { align: 'center' });

    doc.setFont('helvetica', 'bold');
    doc.setFontSize(18);
    doc.text(`${reportData.monthLabel}.`, textCenterX, ty + 15, { align: 'center' });
    ...
    for (const [platform, data] of Object.entries(reportData.platforms)) {
      ...
      narrative.split('\n').forEach(line => {
        ...
        const wrapped = doc.splitTextToSize(line, cw);
        wrapped.forEach(wl => {
          if (y > ph - 5) { doc.addPage(); pdfDrawBg(doc, pw, ph); y = margin; }
          doc.text(wl, margin, y);
          y += 4.5;
        });
        y += 1;
      ...
      if (cc && cc.images && cc.images.length > 0) {
        y += 3;
        if (y + cc.h > ph - 5) { doc.addPage(); pdfDrawBg(doc, pw, ph); y = margin; }
        const totalImgW = cc.images.length * cc.w + (cc.images.length - 1) * 5;
        ...
        cc.images.forEach((src, i) => {
          ...
          if (cached) {
            try {
              ...
              if (b64) {
                if (imgX + cc.w > pw - margin) { imgX = margin; y += cc.h + 5; }
                if (y + cc.h > ph - 5) { doc.addPage(); pdfDrawBg(doc, pw, ph); y = margin; }
