// ============== DATA ACCESS LAYER ==============
// Wraps Supabase queries behind the same function signatures as localStorage.
// Uses in-memory cache for synchronous reads after prefetch.
...
async function fetchClients() {
  ...
  if (error) {
    ...
    notify('Greška pri učitavanju klijenata: ' + error.message, 'warning');
    return {};
  }

  // Use a Map-like ordered object: insert in desired display order
  const clientMap = {};
  data.forEach(row => {
    clientMap[row.id] = {
      name: row.name,
  ...
  // Maintain preferred display order (existing clients first, new ones appended)
  const preferredOrder = ['nlb', 'urban', 'krka'];
  const clients = {};
  preferredOrder.forEach(id => { if (clientMap[id]) clients[id] = clientMap[id]; });
  // Append any new clients not in preferredOrder
...
// ============== PREFETCH ==============
// Loads all data for a client into cache in one batch

const _prefetchInProgress = {};

async function prefetchClientData(clientId) {
  // Guard: prevent parallel prefetch calls for the same client
  ...
  try {
    // Fetch campaign data with pagination (fetch BEFORE clearing cache)
    ...
    const [budgetRes, flightRes, ga4Res] = await Promise.all([
      ...
      sb.from('flight_days')
        .select('*')
        .eq('client_id', clientId),
      sb.from('ga4_kpi_data')
        .select('*')
        .eq('client_id', clientId)
    ]);

    // Clear cache AFTER all fetches complete (no async gap between clear and populate)
    ...
    // Populate cache (immediately after clear, no async gap)
...
// Prefetch all clients' homepage data
async function prefetchHomepageData() {
  const clients = _cache.clients || await fetchClients();
  ...
  while (true) {
    ...
    if (error) {
      ...
      notify('Greška pri učitavanju podataka: ' + error.message, 'warning');
      break;
    }
  ...
  // Clear campaign cache before rebuilding
  _cache.campaignData = {};
...
// ============== CAMPAIGN DATA (read from cache) ==============

function dbGetCampaignData(clientId, platform, month) {
  const key = `${clientId}_${platform}_${month}`;
  return _cache.campaignData[key] || [];
}

async function dbSaveCampaignData(clientId, platform, month, rows) {
  const key = `${clientId}_${platform}_${month}`;

  // Deduplicate by unique key (date + campaign + insertion_order)
  // CSV can have multiple rows with same key — aggregate them
  const deduped = {};
  rows.forEach(r => {
    const date = r.date || '';
  ...
  // Update cache with deduped data
  _cache.campaignData[key] = cleanRows;
  console.log(`[dbSave] ${key}: ${rows.length} raw → ${cleanRows.length} deduped`);

  // Build Supabase records
  const records = cleanRows.map(r => ({
    client_id: clientId, platform, month, ...r
  }));

  // Delete existing rows for this client/platform/month, then insert new
  ...
  if (delError) {
    console.error('[dbSave] DELETE error:', delError.message, '— podaci ostaju u kešu ali nisu sačuvani u bazi');
    return; // Don't proceed with insert if delete failed
  }
  ...
  if (records.length > 0) {
    ...
    if (insertFailed) {
      console.warn('[dbSave] Insert failed for', key, '— podaci su u kešu ali NE u Supabase bazi.');
    }
  }
}

// ============== BUDGETS (read from cache) ==============

function dbGetBudget(clientId, platform, month) {
  return _cache.budgets[`budget_${clientId}_${platform}_${month}`] || 0;
}

async function dbSetBudget(clientId, platform, month, amount) {
...
// ============== FLIGHT DAYS (read from cache) ==============

function dbGetFlightDays(clientId, month) {
  return _cache.flightDays[`flight_${clientId}_${month}`] || [];
}

async function dbSetFlightDays(clientId, month, days) {
...
function dbGetGA4Data() {
  // Return all GA4 data grouped by month (same format as localStorage version)
  const result = {};
  Object.entries(_cache.ga4Data).forEach(([key, rows]) => {
    // key format: ga4_clientId_month
...
async function dbSaveGA4Data(clientId, month, rows) {
  ...
  if (rows.length > 0) {
    ...
    const { error } = await sb.from('ga4_kpi_data')
      .upsert(records, { onConflict: 'client_id,month,product' });
...
async function dbGetSheetLinks() {
  ...
  if (error) {
    console.error('fetchSheetLinks error:', error);
    return {};
  }
...
async function dbSaveSheetLinks(links) {
  _cache.sheetLinks = links;
  ...
  for (const rec of records) {
    const { error } = await sb
      .from('sheet_links')
      .upsert(rec, { onConflict: 'client_id,platform' });
...
function dbGetAllCampaignKeys(clientId, platform) {
  // Returns all month keys matching a client_platform prefix
...
function dbGetAllCampaignDataForPlatform(clientId, platform) {
  const prefix = `${clientId}_${platform}_`;
  let allRows = [];
...
async function dbSetClientAccess(userId, clientId, grant) {
  if (grant) {
...
async function runDiagnostics() {
  ...
  if (!results.httpsOk) console.warn('[Diagnostics] ⚠ HTTP detektovan — Supabase zahteva HTTPS!');

  // 2. CDN check
  results.chartJs = typeof Chart !== 'undefined';
  results.supabaseCdn = typeof window.supabase !== 'undefined';
  results.sbClient = typeof sb !== 'undefined';
  ...
  // 4. User role
  try {
    const { data, error } = await sb.from('user_profiles').select('role, full_name, email').limit(5);
    results.userProfiles = data || [];
    results.userProfilesError = error?.message || null;
    if (data && data.length > 0) {
      const myProfile = data.find(p => p.email === results.session?.email);
      results.myRole = myProfile?.role || 'NOT FOUND';
      if (results.myRole === 'viewer') console.warn('[Diagnostics] ⚠ Tvoja rola je "viewer" — nemaš pristup podacima! Promeni na "admin" u Supabase SQL Editor.');
    }
  } catch (e) { results.userProfilesError = e.message; }

  // 5. Table access tests
  ...
  for (const table of tables) {
    try {
      ...
      results.tables[table] = { count: count, error: error?.message || null };
    } catch (e) { results.tables[table] = { count: 0, error: e.message }; }
  }

  // 6. Sheet links test
  try {
    const { data } = await sb.from('sheet_links').select('client_id, platform, sheet_url');
    results.sheetLinks = (data || []).map(r => ({ key: `${r.client_id}_${r.platform}`, url: r.sheet_url?.substring(0, 60) + '...' }));
  } catch (e) { results.sheetLinksError = e.message; }
  ...
  console.log('Protocol:', results.protocol, results.httpsOk ? '✓' : '✗ HTTPS POTREBAN');
  console.log('CDN:', results.chartJs ? 'Chart.js ✓' : 'Chart.js ✗', results.supabaseCdn ? 'Supabase ✓' : 'Supabase ✗');
  console.log('Session:', results.session || 'NEMA SESIJE');
  console.log('Moja rola:', results.myRole || 'nepoznata');
  console.log('Cache:', results.cache);
  console.log('Sheet linkovi:', results.sheetLinks);
  console.log('%c[Diagnostics] Gotovo. Pošalji screenshot ovih rezultata za pomoć.', 'color:#22c55e;font-weight:bold;');
  return results;
}
