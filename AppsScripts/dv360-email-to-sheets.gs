...
const CONFIG = {
  ...
  SHEET_ID: 'OVDE_UNESI_SHEET_ID',

  // Nazivi tabova u Sheet-u
  TABS: {
    'NLB banka': 'NLB DV360',
    'Krka_RS_DV360': 'Krka DV360'
  },
  ...
  HEADERS: ['Date', 'Campaign', 'Insertion Order', 'Impressions', 'Reach', 'Clicks', 'Cost'],
      ...
      for (const attachment of attachments) {
        ...
        for (const [advertiser, advRows] of Object.entries(grouped)) {
          ...
          if (advertiser === CONFIG.KRKA_FILTER.advertiser) {
            ...
            Logger.log('Krka filter: ' + advRows.length + ' → ' + filteredRows.length + ' redova');
          }
...
function parseCSV(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (line.startsWith('Report Time') || line.startsWith('Date Range') ||
        line.startsWith('Group By') || line.startsWith('MRC') ||
        line.startsWith('Filter') || line.startsWith('"')) break;
...
function filterKrkaRows(rows) {
  return rows.filter(row => {
    ...
    const hasInclude = CONFIG.KRKA_FILTER.include.some(term => combined.includes(term));

    // Ne sme sadržati nijedan exclude termin
    const hasExclude = CONFIG.KRKA_FILTER.exclude.some(term => combined.includes(term));
...
function writeToSheet(ss, tabName, rows) {
  ...
  const newRows = rows.map(row => {
    ...
    return [date, campaign, insertionOrder, impressions, reach, clicks, parseFloat(cost.toFixed(2))];
  });
  ...
  const existingDates = new Set(existingData.map(d => {
  ...
  // Append nove redove
  if (filteredRows.length > 0) {
    sheet.getRange(sheet.getLastRow() + 1, 1, filteredRows.length, CONFIG.HEADERS.length)
