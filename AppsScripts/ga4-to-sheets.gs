...
const GA4_CONFIG = {
  ...
  SPREADSHEET_ID: 'OVDE_STAVI_SPREADSHEET_ID',
  TAB_NAME: 'GA4 KPI',

  // Proizvodi i njihovi URL-ovi
  PRODUCTS: {
    'Keš krediti': [
      'https://www.nlbkb.rs/stanovnistvo/krediti/kes-krediti'
    ],
    'Keš online': [
    ...
    'Stambeni': [
      'https://www.nlbkb.rs/stanovnistvo/krediti/stambeni-krediti'
    ],
    'Zeleni stambeni': [
  ...
  OUTPUT_ORDER: [
    ...
    'Keš bez dokumentacije', 'Keš pokriven depozitom', 'Keš do 100k',
    'UKUPNO Keš',
    'Stambeni', 'Zeleni stambeni', 'Zeleni', 'Računi'
  ],
...
function processPreviousMonth() {
  ...
  for (const [product, urls] of Object.entries(GA4_CONFIG.PRODUCTS)) {
    ...
    const useLeads = GA4_CONFIG.CPC_ONLY_PRODUCTS.includes(product) ? data.leadsCpc : data.leads;
    allResults.push({
      month: monthLabel,
      product: product,
      leads: useLeads,
...
function runGA4Report({ startDate, endDate, metrics, dimensions, dimensionFilter }) {
  const request = {
  ...
  try {
    return AnalyticsData.Properties.runReport(request, `properties/${GA4_CONFIG.PROPERTY_ID}`);
  } catch (e) {
...
function writeToSheet(results) {
  ...
  // Upiši nove redove
  const newRows = results.map(r => [r.month, r.product, r.leads, r.sessions, r.users]);
