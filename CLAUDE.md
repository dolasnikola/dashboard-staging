...
### Data Layer (`src/lib/`)
- `supabase.js` — Supabase client init via `@supabase/ssr` `createBrowserClient`, exports `sb` (no longer used directly — all calls go through `api.js` proxy)
- `api.js` — Frontend wrapper for all Supabase calls via `/api/*` proxy routes. Exports: `apiLogin`, `apiLogout`, `apiGetUser`, `apiUpdatePassword`, `dbQuery`, `dbSelect`, `dbInsert`, `dbUpdate`, `dbDelete`, `dbUpsert`, `rpcCall`, `storageUpload`, `storageCreateSignedUrl`, `storageRemove`
...
- `authStore.js` — Zustand: currentUser, currentUserRole, login/logout/checkSession. Uses `apiLogin`/`apiLogout`/`apiGetUser` from `api.js` (httpOnly cookie auth). Auth state changes detected via visibility polling (no direct token access).
...
### Vercel API Proxy (`api/`)
All frontend Supabase calls go through `/api/*` serverless routes. JWT lives in httpOnly cookies.
```
_lib/supabase-server.mjs    — createServerClient from @supabase/ssr, cookie parsing, httpOnly cookie setter
auth/login.mjs              — POST: email/password → signInWithPassword → set httpOnly cookie
auth/logout.mjs             — POST: signOut → clear cookie
auth/user.mjs               — GET: getUser from cookie → return user
auth/update.mjs             — POST: updateUser (password change)
auth/callback.mjs           — GET: PKCE auth code exchange
db.mjs                      — POST: generic DB proxy (select/insert/update/delete/upsert) with table whitelist
rpc.mjs                     — POST: RPC proxy with function whitelist
storage.mjs                 — POST: Storage proxy (upload/createSignedUrl/remove) with bucket whitelist
```
**Security:** Table/RPC/bucket whitelists. RLS still enforced via JWT from cookie. Domain restriction via cookie settings (currently auto-restricted to exact Vercel URL; for custom domain add `Domain=` in `api/_lib/supabase-server.mjs:37`).

### Components (`src/components/`)
```
auth/LoginGate.jsx          — Email/password login form
auth/SetPassword.jsx        — Password update form (uses proxy)
layout/Layout.jsx           — Main layout wrapper with Sidebar + content area
layout/Sidebar.jsx          — Left sidebar navigation, collapsible, auto-hide on mobile scroll
...
## Auth & Security
- **Supabase Auth** with email/password, **httpOnly cookie proxy** (FAZA 5 security hardening, deployed 2026-04-03)
- **Session protection:** JWT stored in httpOnly Secure SameSite=Lax cookies — JavaScript cannot read tokens. All Supabase calls proxied through Vercel API routes (`api/`). XSS cannot steal session.
...
- **Security docs:** `cyber.md` — full analysis of auth approaches, implementation details, and TODO list
...
- react, react-dom, react-router-dom, @supabase/supabase-js, **@supabase/ssr**, chart.js, react-chartjs-2, zustand, jspdf, jspdf-autotable, @tanstack/react-virtual
...
## Gotchas
- **Frontend never calls `sb` directly** — all Supabase calls go through `src/lib/api.js` → `/api/*` proxy → httpOnly cookie auth. `sb` exists in `supabase.js` but is unused.
...
- **Navigation is Sidebar, not Header** — `Header.jsx` deleted, replaced by `Layout.jsx` + `Sidebar.jsx`. Sidebar collapses on mobile, auto-hides on scroll.
...
- **Security Hardening — httpOnly Cookie Proxy (DONE):** JWT moved from localStorage to httpOnly cookies. All Supabase calls proxied through Vercel API routes. XSS session hijacking eliminated. `@supabase/ssr` + `api/` directory. Deployed 2026-04-03.
- **Sidebar Navigation (DONE):** Top header replaced with collapsible left sidebar. Auto-hide on mobile scroll, reappear on scroll stop. `Layout.jsx` + `Sidebar.jsx` replace `Header.jsx`. Deployed 2026-04-03.
...
- Security: CSP headers (additional XSS layer)
- Security: MFA/TOTP (second auth factor)
- Security: Cookie `Domain=` restriction (when custom domain added)
...
- `AlertBell.jsx` in Sidebar: unread count badge, dropdown with dismiss/mark-read
